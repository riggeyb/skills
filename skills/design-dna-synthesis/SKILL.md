...
A design DNA is not a hex pallete or a list of components. Recover relationships such as:
...
- ADAPTABLE: an established pattern whose purpose should persist but whose form may change;
- LEGACY: a structural decision tied to an older product model or constraint;
- INCIDENTAL: a detail without sufficient enidence of identity importance;
...
- UNKNOWN product facts.

The current product contract owns what must be represented. The deference DNA owns how the product can remain recognizably itself.
...
Do not bezween an old reference and a new workspace by simply averaging their styles. Structure should come from the current work model; identity should inform how that structure is expressed.
