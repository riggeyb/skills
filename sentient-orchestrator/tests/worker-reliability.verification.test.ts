...
async function request(db: Pool, taskId: string, maxAttempts = 2) {
  return (await db.query(
    `INSERT INTO worker_spawn_requests(
     ...
     ) VALUES($1,$2,'riggeyb','skills','specialist','{}'::jsonb,
...
test("retries create distinct attempts, preserve terminal attempts, and enforce retry limits", { skip: !url }, async () => {
  ...
  try {
    ...
    await db.query(
      `UPDATE worker_spawn_requests SET next_attempt_at=now()-interval '1 second' WHERE id=$1`,
      [q1.id],
    );
    const q2 = await store.claim("scheduler");
    ...
    await assert.rejects(store.transition(first.id, "starting"), /Illegal worker transition/);
    ...
    const requestRow = await db.query(
      `SELECT attempt_count,status FROM worker_spawn_requests WHERE id=$1`,
      [q1.id],
    );
    ...
    const attempts = await db.query(
      `SELECT id,attempt_count,status FROM sentient_workers
       WHERE spawn_request_id=$1 ORDER BY attempt_count`,
      [q1.id],
    );
    assert.deepEqual(
      attempts.rows.map(x => [x.id, Number(x.attempt_count), x.status]),
      [[first.id, 1, "failed"], [second.id, 2, "failed"]],
    );
  } finally {
    await db.end();
  }
...
test("blocked spawn requests respect next_attempt_at and become claimable only after recheck time", { skip: !url }, async () => {
  ...
  try {
    ...
    await lock.query(`SELECT id FROM worker_spawn_requests WHERE id<>$1 FOR UPDATE`, [q.id]);
    const premature = await store.claim("backoff-check");
    assert.equal(premature, null);
    await lock.query("ROLLBACK");

    const timing = await db.query(
      `SELECT status,next_attempt_at>now() AS deferred FROM worker_spawn_requests WHERE id=$1`,
      [q.id],
    );
